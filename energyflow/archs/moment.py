...
def construct_distributed_dense(input_tensor, sizes, acts='relu', k_inits='he_uniform', 
                                                                  names=None, l2_regs=0., l1_regs=0., use_bias=True):
    """"""
    ...
    for s, act, k_init, name, l2_reg, l1_reg in zip(sizes, acts, k_inits, names, l2_regs, l1_regs):
        
        # define a dense layer that will be applied through time distributed
        kwargs = {} 
        if l1_reg > 0.:
            kwargs.update({'kernel_regularizer': l1(l1_reg), 'bias_regularizer': l1(l1_reg)})
        elif l2_reg > 0.:
...
class SymmetricPerParticleNN(NNBase):
    ...
    def _construct_F(self):
        ...
        F_layers, F_tensors = construct_dense(self.tensors[-1], self.F_sizes,
                                              ...
                                              l2_regs=self.F_l2_regs, l1_regs = self.F_l1_regs)
